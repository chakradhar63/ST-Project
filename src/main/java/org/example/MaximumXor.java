...
public class MaximumXor {
    public static ArrayList<Integer> maxXorQueries(ArrayList<Integer> arr, ArrayList<ArrayList<Integer>> queries) {
        ...
        for (ArrayList<Integer> q1 : queries) {
        ...
        for (ArrayList<Integer> query : q) ans.add(-1);

        for (ArrayList<Integer> query : q) {
            int num = query.get(1);
            int numCheck = query.get(0);
            int ind1 = query.get(2);

            for (int i = ind; i < arr.size(); i++) {
                if (arr.get(i) <= numCheck) {
                    insert(arr.get(i), trie.root);
                } else {
                    break;
                }
                ind++;
